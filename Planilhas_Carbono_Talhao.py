...
PASTA_SAIDAS = Path(
    r"C:\Users\migue\OneDrive - Agrorobotica Fotonica Em Certificacoes Agroambientais\_Fertilidade\ENTRADAS\SAIDA_TESTE"
)

ARQ_EMPRESA = r"C:\Users\migue\OneDrive - Agrorobotica Fotonica Em Certificacoes Agroambientais\_Fertilidade\ENTRADAS\ENTRADA_TESTE\F20251711S-UNICO.xlsx"
...
SPEC_ABA1 = {
    ...
    "ID_TALHAO": ["Talhão Agrorobotica", "Talhao Agrorobotica"],
    "TALHAO_COMERCIAL": ["Talhão Comercial", "Talhao Comercial"],
    "ID_PONTO": ["Ponto de Coleta", "Ponto Coleta"],
    "PROFUNDIDADE": ["Profundidade (cm)", "Profundidade cm"],
...
def read_aba1_quimica(caminho: Path) -> pd.DataFrame:
    ...
    ensure_numeric(df, [
        "pH_CaCl2", "pH_agua",
        "P_(meh)_(mg/dm³)", "P_(res)_(mg/dm³)",
        "S_(mg/dm³)", "MOS_(g/dm³)",
        "Ca_(cmolc/dm³)", "Mg_(cmolc/dm³)", "K_(cmolc/dm³)", "K_(mg/dm³)",
        "Al_(cmolc/dm³)", "H+Al_(cmolc/dm³)",
    ])
    ...
    kk = df["K_(cmolc/dm³)"]
    al = df["Al_(cmolc/dm³)"]
    hal = df["H+Al_(cmolc/dm³)"]

    df["SB_(cmolc/dm³)"] = (ca + mg + kk).round(3)
    df["CTCp_(cmolc/dm³)"] = (df["SB_(cmolc/dm³)"] + hal).round(3)
    df["CTCe_(cmolc/dm³)"] = (df["SB_(cmolc/dm³)"] + al).round(3)

    # Nome que o template espera:
    df["T_(cmolc/dm³)"] = df["CTCp_(cmolc/dm³)"]

    df["V_(%)"] = pd.to_numeric(frac_pct(df["SB_(cmolc/dm³)"], df["CTCp_(cmolc/dm³)"]), errors="coerce").round(2)
    df["Sat._Al_(%)"] = pd.to_numeric(frac_pct(al, df["CTCe_(cmolc/dm³)"]), errors="coerce").round(2)

    df["Ca/CTC_(%)"] = pd.to_numeric(frac_pct(ca, df["CTCp_(cmolc/dm³)"]), errors="coerce").round(2)
    df["Mg/CTC_(%)"] = pd.to_numeric(frac_pct(mg, df["CTCp_(cmolc/dm³)"]), errors="coerce").round(2)
    df["K/CTC_(%)"] = pd.to_numeric(frac_pct(kk, df["CTCp_(cmolc/dm³)"]), errors="coerce").round(2)
    df["H+Al/CTC_(%)"] = pd.to_numeric(frac_pct(hal, df["CTCp_(cmolc/dm³)"]), errors="coerce").round(2)

    df["Ca/Mg"] = pd.to_numeric(safe_div(ca, mg), errors="coerce").round(2)
    df["Ca/K"] = pd.to_numeric(safe_div(ca, kk), errors="coerce").round(2)
    df["Mg/K"] = pd.to_numeric(safe_div(mg, kk), errors="coerce").round(2)
...
# ===================== PIPELINE =====================
def main():
    caminho_p1 = PASTA_ENTRADAS / ARQ_EMPRESA
    caminho_tpl = PASTA_ENTRADAS / ARQ_TEMPLATE
    caminho_end = PASTA_ENTRADAS / ARQ_ENDERECOS

    if not caminho_p1.exists():
        print(f"[ERRO] Não encontrei a planilha-empresa: {caminho_p1}")
        sys.exit(1)
    if not caminho_tpl.exists():
        print(f"[ERRO] Não encontrei o template: {caminho_tpl}")
        sys.exit(1)
    if not caminho_end.exists():
        print(f"[ERRO] Não encontrei ENDERECOS: {caminho_end}")
        sys.exit(1)

    base_stem = Path(ARQ_EMPRESA).stem

    # SUST
    arq_sust_stem = base_stem.replace("-UNICO", "-SUST")
    caminho_sust = PASTA_ENTRADAS / f"{arq_sust_stem}.xlsx"
    if not caminho_sust.exists():
        print(f"[ERRO] Não encontrei o SUST: {caminho_sust}")
        sys.exit(1)
    nome_sust_sem_sufixo = arq_sust_stem.replace("-SUST", "")
    ...
    # >>> Correção 171: ID_PONTO veio como profundidade
    df1, qr_to_ponto = fix_ponto_when_depthlike(df1)
    df2 = apply_qr_point_map(df2, qr_to_ponto)
    df3 = apply_qr_point_map(df3, qr_to_ponto)

    # Merge (ABA1 base)
    df_all = merge_left(df1, df2, KEYS_MERGE, "MERGE ABA1+ABA2")
    df_all = merge_left(df_all, df3, KEYS_MERGE, "MERGE (ABA1+ABA2)+ABA3")

    # Ordenação final (inclui talhão comercial alfabético)
    df_all = sort_dataframe_for_output(df_all)
    ...
    # Consolidado
    nome_consolidado = base_stem.replace("-UNICO", "") + ".xlsx"
    caminho_consolidado = PASTA_SAIDAS / nome_consolidado
    write_group_to_template(df_all, caminho_tpl, caminho_consolidado, df_sust_map, nome_sust_sem_sufixo, endereco_val)
    print(f"[OK] Gerado consolidado: {caminho_consolidado} ({len(df_all)} linhas)")
